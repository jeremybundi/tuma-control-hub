...
const VerifyOTPContent = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        "https://tuma-dev-backend-auth-alb-2099885708.us-east-1.elb.amazonaws.com/api/auth/email",
        {
          email: email,
  ...
  const handleResendOTP = async () => {
    ...
    try {
      // Consider moving this URL to an environment variable
      await axios.post(
        `https://tuma-dev-backend-auth-alb-2099885708.us-east-1.elb.amazonaws.com/api/auth/send-otp/${encodeURIComponent(email)}`
      );
